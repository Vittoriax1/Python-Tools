...
def normalize_url(url):
    ...
    if parsed.netloc.startswith("www."):
        return url
    return f"{parsed.scheme}://www.{parsed.netloc}{parsed.path}"
...
def crawl(url, depth, results_box, tree, parent_node):
    ...
    if detections:
        results_box.insert(tk.END, f"[FOUND] {url}\n  - {', '.join(detections)}\n")
    else:
        results_box.insert(tk.END, f"[OK] {url} — no detections\n")
    ...
    for link in links:
        ...
        if parsed.netloc == domain and new not in visited:
            ...
            crawl(new, depth - 1, results_box, tree, node)


def threaded_scan(url_entry, depth_entry, results_box, tree):
    def run():
        ...
        crawl(url, depth, results_box, tree, root)
...
def run_gui():
    ...
    start_btn = tk.Button(win, text="Start Scan", command=lambda:
                          threaded_scan(url_entry, depth_entry, results_box, tree))
