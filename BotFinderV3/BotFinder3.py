...
def crawl(url, depth, results_box, tree, parent_node, found_box):
    ...
    # --- Hybrid: if no detections and JS mode enabled, try Playwright ---
    network_urls = []
    if js_mode and PLAYWRIGHT_AVAILABLE and not detections:
        results_box.insert(tk.END, f"[JS] Re-checking with headless browser: {url}\n", "crawl")
        ...
        if html_js:
            ...
            detections = detect_ai_widgets(scripts_js, network_urls)
    ...
    # Crawl links (HTML DOM only; that's enough to discover URLs)
...
def threaded_scan(url_entry, depth_entry, results_box, tree, found_box, js_var):
    def run():
        global stop_flag, pause_flag, js_mode
        stop_flag = False
        pause_flag = False
        ...
        mode_label = "Hybrid (HTML+JS)" if js_mode else "HTML-only"
...
def run_gui():
    ...
    win = tk.Tk()
    win.title("AI/Chatbot Scanner - Hybrid")
